...
pub fn SPMCQueue(comptime block_size: u32) type {
    return struct {
        ...
        pub fn init(blocks_buffer: []Block) @This() {
            return @This(){
                .header = .empty,
        ...
        pub fn read(self: *@This(), block_index: u64, dest_buffer: *[block_size]u8) ?[]const u8 {
            var block = &self.blocks[block_index];
...
test "contention is fine" {
    const block_size = 32;
    const Queue = SPMCQueue(block_size);

    const write = struct {
        pub fn func(is_running: *std.atomic.Value(bool), queue: *Queue) void {
            var id: u64 = 0;
            var msg_buf: [block_size]u8 = undefined;
    ...
    const read = struct {
        pub fn func(is_running: *std.atomic.Value(bool), idx: usize, queue: *Queue, msg_count: *std.atomic.Value(u64)) void {
            var block_index: u32 = 0;
            var dest_buffer: [block_size]u8 = undefined;
            while (is_running.load(.acquire)) {
    ...
    var blocks: [1024]Queue.Block = undefined;
    var queue = Queue.init(&blocks);
